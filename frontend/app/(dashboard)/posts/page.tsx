...
export default function PostsPage() {
  ...
  const loadPosts = async () => {
    try {
      const response = await fetch('/api/gateway/admin/posts/with-polls', {
        credentials: 'include',
      });
  ...
  const handleDelete = async (postId: number) => {
    ...
    try {
      const response = await fetch(`/api/gateway/admin/posts/${postId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
                    ...
                    {poll && poll.question && Array.isArray(poll.options) && (
                      <div className="mt-4 p-4 bg-gradient-to-br from-purple-50 to-blue-50 rounded-lg border border-purple-200">
                            ...
                            return (
                              <div key={option.id} className="relative">
                                <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200 relative overflow-hidden">
                                  ...
                                  />
                                  <span className="relative z-10 font-medium text-gray-900">{option.text}</span>
                        ...
                        <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
                          <span>Всего голосов: {poll.options.reduce((sum: number, opt: any) => sum + (opt.votes_count || 0), 0)}</span>
