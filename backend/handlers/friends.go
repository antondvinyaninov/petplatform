...
func SendFriendRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	`)
	err := db.DB.QueryRow(query, userID, req.FriendID, req.FriendID, userID).Scan(&existingID)
	...
	`)
	var id int64
	err = db.DB.QueryRow(query, userID, req.FriendID, time.Now(), time.Now()).Scan(&id)
	...
	`)
	err = db.DB.QueryRow(query, userID).Scan(&senderName, &senderLastName)
	...
	CreateUserLog(db.DB, userID, "friend_request_send", "Отправлен запрос в друзья", ipAddress, userAgent)
...
func AcceptFriendRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	`)
	result, err := db.DB.Exec(query, time.Now(), req.FriendshipID, userID)
	...
	query = convertPlaceholdersFriends(`
		...
		WHERE f.id = ?
	`)
	err = db.DB.QueryRow(query, userID, req.FriendshipID).Scan(&senderID, &acceptorName, &acceptorLastName)
	...
	ipAddress := r.RemoteAddr
	userAgent := r.Header.Get("User-Agent")
	CreateUserLog(db.DB, userID, "friend_request_accept", "Принят запрос в друзья", ipAddress, userAgent)
...
func RejectFriendRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := convertPlaceholdersFriends(`
		...
		WHERE id = ? AND friend_id = ? AND status = 'pending'
	`)
	result, err := db.DB.Exec(query, req.FriendshipID, userID)
...
func RemoveFriendHandler(w http.ResponseWriter, r *http.Request) {
	...
	`)
	result, err := db.DB.Exec(query, req.FriendshipID, userID, userID)
...
func GetFriendshipStatusHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := convertPlaceholdersFriends(`
		...
		WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)
	`)
	err = db.DB.QueryRow(query, userID, friendID, friendID, userID).Scan(
